...
function read_resource_values() {
  local index=$1
  awk -v i="$index" -F, 'NR==i+1 {print $2, $3, $4, $5}' blender-resources.csv
}
...
function run_test() {
  local index=$1
  local yaml_file="blender-${index}.yaml"
  ...
  kubectl exec -i -t blender -- sh ./phoronix_test.sh >> blender-result.txt & wait $!

  kubectl delete pod blender --wait

  sleep 60s

  echo "===${index} result ==="
  cat blender-result.txt | grep Average
}
